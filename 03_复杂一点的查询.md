...
order by product_type;  -- mysql中允许view中使用order by

SELECT * FROM productsum;
```
![image-20210102223253644](C:/Users/keen/AppData/Roaming/Typora/typora-user-images/image-20210102223253644.png)

为什么不能使用 ORDER BY 子句呢？这是因为视图和表一样，**数据行都是没有顺序的**。

> 但可以对查出来的结果进行排序。

在 MySQL中视图的定义是允许使用 ORDER BY 语句的，但是若从特定视图进行选择，而该视图使用了自己的 ORDER BY 语句，则视图定义中的 ORDER BY 将被忽略，**即order by只有最后一个会起作用。**
...
```sql
CREATE VIEW view_shop_product(proudct_type, sale_price, shop_name)
AS
SELECT product.`product_type`, product.sale_price,shopproduct.shop_name
FROM product,shopproduct
WHERE product.`product_id` = shopproduct.product_id;
```
创建的视图如下图所示：

![image-20210102224421954](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102224422.png)
...
WHERE view_shop_product.`proudct_type` = '衣服';
```
查询结果为：

![image-20210102224525566](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102224525.png)
...
AS
SELECT product_type, sale_price -- 视图结构从(product_type,cnt_product)
FROM product
WHERE product.`regist_date` > '2009-09-11';
```
...
![image-20210102230621602](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102230621.png)

```sql
SELECT * FROM product;
```

![image-20210102230835361](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102230835.png)

```sql
-- 在product表中插入一条数据
INSERT INTO product VALUES('010','筷子','厨房用品',100,50,'2021-01-02');
```

此时视图中内容会一致的更新：

![image-20210102230928080](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102230928.png)

```sql
-- 往视图中插入数据会比较困难，因为一般比较难以满足其所依赖表的限制
-- 故也不推荐在视图中插入数据，
-- 视图中更新了数据，也会同步到相应表中。
```
...
> update是更新数据记录，而alter是更改表或视图的结构。
...
![image-20210102232142613](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102232142.png)
...
子查询指一个查询语句嵌套在另一个查询语句内部的查询，这个特性从 MySQL 4.1 开始引入，在 SELECT 子句中先计算子查询，子查询结果作为外层另一个查询的过滤条件，子查询可以基于一个表或者多个表。
...
![image-20210102232745685](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102232745.png)
...
```
![image-20210102233022340](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102233022.png)
...
FROM product;
```
![image-20210102233136732](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210102233136.png)
...
关联子查询相比于子查询之间多了 存在着联系。这种联系是如何建立起来的呢？
...
> 上边代码要的结果是：  大于各自类别平均值的product有哪些？
...
**（1）先从主查询的Product表中product _type列取出第一个值（没有执行where），进入子查询中，得到子查询结果，然后返回父查询，判断父查询的where子句条件，则返回整个语句的第1条结果。**
...
FROM product
WHERE sale_price>=1000 AND product.`regist_date`='2009-09-20';
...
```
![image-20210103001433907](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210103001434.png)
...
```sql
-- 做了很多更改操作，因为之前的一些操作修改了表
CREATE VIEW sale_price_all
AS
SELECT product_id,product_name,product_type,sale_price,
(SELECT SUM(sale_price) FROM product) AS sale_price_all
FROM product;

ALTER VIEW sale_price_all
AS
SELECT product_id,product_name,product_type,sale_price,
(SELECT AVG(sale_price) FROM product) AS sale_price_all
FROM product;

DELETE FROM sale_price_all WHERE product_id='010';

DELETE FROM product WHERE product_id='010';

UPDATE product SET sale_price='500' WHERE product_id='0002';


UPDATE product SET sale_price='100' WHERE product_id='0008';
SELECT * FROM sale_price_all;
```
...
![image-20210117112942667](C:/Users/keen/AppData/Roaming/Typora/typora-user-images/image-20210117112942667.png)
...
错，不会全变成NULL；而是会能运算的运算，为NULL的不管。
...
```
![image-20210118181335288](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210118181335.png)
...
```
![image-20210118181138220](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210118181138.png)

答案：

语句①返回进货价格非空且不包含在500、2800、5000的商品名称及其进货价格。

语句②不返回任何结果。因为任何值与NULL做比较，结果都为NULL。可以理解为NULL不能进行比较，所以筛选不出来任何记录。
...
![image-20210118202145633](https://cdn.jsdelivr.net/gh/lizhangjie316/img/2020/20210118202145.png)